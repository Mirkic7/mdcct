# Implementation of Shabal: i386 with SSE2
#
...
Li1:
	...
	movl    12(%esp), %edi         # Load context address in %edi.
	movl    16(%esp), %eax
	movl    %eax, 252(%edi)        # Store out_size field.
	shrl    $5, %eax
	imull   $176, %eax, %eax
	addl    %eax, %esi
...
# Input:
#   %ebp   points to the source structure
...
#
# If %ebp is such that the context A[0] address is already equal to
# 16 modulo 64, then this routine sets %edi to &A[0] and returns.
# Otherwise, it copies the state words (the 44 A/B/C words and the
# W counter) into the destination buffer, and keeps %edi unchanged.
# Either way, %edi, upon exit, points to a properly aligned buffer.
#
# Registers other than %edi are preserved.
#
...
# Input:
#   %ebp   points to the context structure
#   %esi   points to the state words
# The copy is performed unless %edi already points to the state
...
shabal:
	...
	movl    16(%esp), %ebp         # Load context pointer in %ebp.
	movl    20(%esp), %esi         # Load data pointer.
	movl    24(%esp), %edx         # Load data length.
...
Ls2:
	# At that point, %edx contains the remaining data length. If
...
Ls3:
	...
	# words is currently in %edi; we save it ans restore it in %esi.
	pushl   %edi
	movl    %ebp, %edi
	movl    %edx, %ecx
	movl    %edx, 64(%ebp)
	...
	# structure. The state words address is in %esi.
...
shabal_close:
	...
	# Conventions:
	#    %ebp   pointer to context structure
	#    %edx   ub
	#    %ecx   n
	#    %esi   output buffer
	movl    20(%esp), %ebp
...
Lc1:
	...
	# We get out_size from the original structure (%ebp) and the C
	# words from the copy (%edi). The destination buffer is
	# currently in %esi.
	movl    252(%ebp), %eax
	movl    %eax, %ecx
	shrl    $3, %eax
